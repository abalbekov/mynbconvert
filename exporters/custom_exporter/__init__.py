...
class AddFilters(HTMLExporter):
    ...
    Filters in this exporter are expected to be called from show-hide-code-output-executetime template.
    To make this available to custom template - place the directory under PYTHONPATH.
    """
    @classmethod
    ...
    header_count_dict=dict( (i+1, 0) for i in range(5) )
    ...
    def number_headers(cls, s):
        ...
        and prepends a number to headers
        and saves the number for next cell iteration
        """
        s_ret=""
